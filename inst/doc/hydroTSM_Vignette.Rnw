...
\date{Oct 2010, Apr 2011, Aug 2011}
...
\item Using the \textit{hydroplot} function, which (by default) plots 9 different graphs: 3 ts plots, 3 boxplots and 3 histograms summarizing 'x'. For this example, only daily and monthly plots are produced, and only data starting from 01-Jan-1987 are plotted.
...
The function \textit{annualfunction} is applied \texttt{FUN} twice over \texttt{x}: 
