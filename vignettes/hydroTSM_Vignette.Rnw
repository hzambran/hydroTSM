...
\label{sec:AdditionalMaterial}

In order to make easier the use of \texttt{hydroTSM} for users not familiar with R, in this section a minimal set of (no well-organised) information is provided to guide the user in the \href{http://www.r-project.org/}{R} world.
