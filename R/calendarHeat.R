...
################################################################################
# Started: 2009 ;                                                              #
# Updates: 25-Nov-2023 ; 27-Nov-2023 ; 29-Nov-2023                             #
################################################################################
...
calendarHeat <- function(x,
                         ...
                         cuts.dec=2,                                        # from Mauricio Zambrano-Bigiarini. Number of decimal places used to present the numbers that divide the range of 'x' in the legend of colours
                         ...
                         cuts.style=c("mzb", "equal", "pretty", "fixed", "sd", "quantile", "kmeans", "bclust", "fisher"), # discarded becsue takes too much time or not alway provide the required number of classes: "dpih", "headtails", "hclust",  "jenks"
    ...
    ######################################################################################
    # Lines 66-94 are from Mauricio Zambrano-Bigiarini
    ######################################################################################
    if (missing(cuts)) {
      ncuts <- length(col) + 1
    ...
    print(
      cal.plot <- lattice::levelplot(value~woty*dotw | yr, data=caldat,
         ...
         colorkey = list(title = paste0(legend.title, " \n"),
                         ...
                         labels=list(labels=cuts.labels, at=as.numeric(factor(lcuts)), cex=1.1), # equally spaced color bins in the legend
                         col=col                                     
                         ) 
         xlab="" ,
         ylab="",
